...
@CucumberResolverExpression("in column {columnName} {webElement} does not contain {stringValue}")
@CucumberResolverExpression("в столбце {columnName} {webElement} не содержит {stringValue}")
public class WebTableCellElementTextStringValueNegativeConditionCucumberResolver  extends AbstractWebTableRowConditionCucumberResolver {
    ...
    public Optional<WebTableRowCondition> tryResolve(@NotNull String expression, @Nullable Object... args) {
        for (Pattern pattern : patterns) {
            ...
            if (matcher.find()) {
                ...
                return Optional.of(notContainText(columnName, elementPath, resolvedStringValue));
            }
        }
        return Optional.empty();
    }
