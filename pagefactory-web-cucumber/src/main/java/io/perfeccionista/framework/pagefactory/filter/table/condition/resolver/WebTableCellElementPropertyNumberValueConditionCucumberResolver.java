...
@CucumberResolverExpression("in column {columnName} property {propertyName} of element {webElement} contains number {numberValue}")
@CucumberResolverExpression("в столбце {columnName} свойство {propertyName} элемента {webElement} содержит число {numberValue}")
public class WebTableCellElementPropertyNumberValueConditionCucumberResolver extends AbstractWebTableRowConditionCucumberResolver {
        ...
        for (Pattern pattern : patterns) {
            ...
            if (matcher.find()) {
                ...
                if (numberValue.contains(".")) {
                    ...
                    return Optional.of(containsProperty(columnName, elementPath, propertyName, resolvedNumberValue));
                }
