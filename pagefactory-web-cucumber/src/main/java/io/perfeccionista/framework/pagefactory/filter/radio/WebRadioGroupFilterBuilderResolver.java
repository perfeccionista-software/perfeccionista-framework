...
import io.perfeccionista.framework.pagefactory.filter.radio.condition.resolver.WebRadioButtonConditionCucumberResolver;
...
import static io.perfeccionista.framework.exceptions.messages.PageFactoryWebCucumberApiMessages.INCORRECT_WEB_RADIO_GROUP_FILTER_BUILDER_DATA_TABLE_FORMAT;
...
public class WebRadioGroupFilterBuilderResolver extends AbstractWebFilterBuilderResolver {

    WebRadioGroupFilterBuilder filterBuilder = null;
    WebRadioButtonCondition currentCondition = null;

    protected WebRadioGroupFilterBuilderResolver(Environment environment, DataTable dataTable) {
        super(environment, dataTable);
    }

    public static WebRadioGroupFilterBuilderResolver of(@NotNull Environment environment, @NotNull DataTable dataTable) {
        return new WebRadioGroupFilterBuilderResolver(environment, dataTable);
    }

    public @NotNull WebRadioGroupFilterBuilder resolve() {
        ...
        if (dataTableAsLists.size() == 1 && dataTableAsLists.get(0).size() == 1) {
            ...
            if (stringEntries.size() <= 2) {
                ...
                throw IncorrectDataTableFormat.exception(INCORRECT_WEB_RADIO_GROUP_FILTER_BUILDER_DATA_TABLE_FORMAT.getMessage());
            }
        }
        ...
        for (List<String> dataTableRow : dataTableAsLists) {
            ...
            WebRadioButtonCondition processedCondition = resolveStringEntry(dataTableRow.get(0));
    ...
    protected void processConditionGrouping(WebRadioButtonCondition processed, WebConditionGrouping grouping) {
    ...
    protected WebRadioButtonCondition resolveStringEntry(String stringEntry) {
        CucumberService cucumberService = environment.getService(CucumberService.class);
        return cucumberService.resolveFirst(WebRadioButtonConditionCucumberResolver.class, stringEntry)
                .orElseThrow(() -> WebFilterConditionNotResolved
                        .exception(WEB_FILTER_CONDITION_NOT_RESOLVED.getMessage(stringEntry, WebRadioGroupFilterBuilder.class.getCanonicalName())));
    }
