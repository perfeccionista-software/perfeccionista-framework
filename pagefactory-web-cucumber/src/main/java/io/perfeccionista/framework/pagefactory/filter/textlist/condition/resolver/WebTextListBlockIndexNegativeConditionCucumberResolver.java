...
import static io.perfeccionista.framework.pagefactory.filter.WebFilterConditions.textBlockIndexNot;
import static java.util.Objects.isNull;

@CucumberResolverExpression("index is not {integerValue}")
@CucumberResolverExpression("индекс не {integerValue}")
public class WebTextListBlockIndexNegativeConditionCucumberResolver extends AbstractWebTextListBlockConditionCucumberResolver {
    ...
    public Optional<WebTextListBlockCondition> tryResolve(@NotNull String expression, @Nullable Object... args) {
        for (Pattern pattern : patterns) {
            ...
            if (matcher.find()) {
                ...
                return Optional.of(textBlockIndexNot(resolvedIntegerValue));
            }
        }
        return Optional.empty();
    }
