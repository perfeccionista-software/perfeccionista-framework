...
@CucumberResolverExpression("in column {columnName} component {component} of element {webElement} is displayed")
@CucumberResolverExpression("в столбце {columnName} компонент {component} элемента {webElement} отображается")
public class WebTableCellElementComponentDisplayedConditionCucumberResolver extends AbstractWebTableRowConditionCucumberResolver {
    ...
    public Optional<WebTableRowCondition> tryResolve(@NotNull String expression, @Nullable Object... args) {
        for (Pattern pattern : patterns) {
            ...
            if (matcher.find()) {
                ...
                return Optional.of(componentDisplayed(columnName, elementPath, component));
            }
        }
        return Optional.empty();
    }
