...
@CucumberResolverExpression("in column {columnName} property {propertyName} of element {webElement} contains {stringValue}")
@CucumberResolverExpression("в столбце {columnName} свойство {propertyName} элемента {webElement} содержит {stringValue}")
public class WebTableCellElementPropertyStringValueConditionCucumberResolver extends AbstractWebTableRowConditionCucumberResolver {
    ...
    public Optional<WebTableRowCondition> tryResolve(@NotNull String expression, @Nullable Object... args) {
        for (Pattern pattern : patterns) {
            ...
            if (matcher.find()) {
                ...
                return Optional.of(containsProperty(columnName, elementPath, propertyName, resolvedStringValue));
            }
        }
        return Optional.empty();
    }
