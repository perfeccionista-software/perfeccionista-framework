...
import static io.perfeccionista.framework.pagefactory.filter.WebFilterConditions.textRowIndexNot;
import static java.util.Objects.isNull;

@CucumberResolverExpression("index is not {integerValue}")
@CucumberResolverExpression("индекс не {integerValue}")
public class WebTextTableRowIndexNegativeConditionCucumberResolver extends AbstractWebTextTableRowConditionCucumberResolver {
    ...
    public Optional<WebTextTableRowCondition> tryResolve(@NotNull String expression, @Nullable Object... args) {
        for (Pattern pattern : patterns) {
            ...
            if (matcher.find()) {
                ...
                return Optional.of(textRowIndexNot(resolvedIntegerValue));
            }
        }
        return Optional.empty();
    }
