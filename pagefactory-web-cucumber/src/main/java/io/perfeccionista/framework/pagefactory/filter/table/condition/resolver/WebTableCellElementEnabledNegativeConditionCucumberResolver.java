...
@CucumberResolverExpression("in column {columnName} {webElement} is disabled")
@CucumberResolverExpression("в столбце {columnName} {webElement} недоступен")
public class WebTableCellElementEnabledNegativeConditionCucumberResolver extends AbstractWebTableRowConditionCucumberResolver {
    ...
    public Optional<WebTableRowCondition> tryResolve(@NotNull String expression, @Nullable Object... args) {
        for (Pattern pattern : patterns) {
            ...
            if (matcher.find()) {
                ...
                return Optional.of(disabled(columnName, elementPath));
            }
        }
        return Optional.empty();
    }
