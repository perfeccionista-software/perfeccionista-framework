...
@CucumberResolverExpression("in column {columnName} component {component} of element {webElement} is not present")
@CucumberResolverExpression("в столбце {columnName} компонент {component} элемента {webElement} отсутствует")
public class WebTableCellElementComponentPresentNegativeConditionCucumberResolver extends AbstractWebTableRowConditionCucumberResolver {
    ...
    public Optional<WebTableRowCondition> tryResolve(@NotNull String expression, @Nullable Object... args) {
        for (Pattern pattern : patterns) {
            ...
            if (matcher.find()) {
                ...
                return Optional.of(componentNotPresent(columnName, elementPath, component));
            }
        }
        return Optional.empty();
    }
