...
//                .shouldHaveIndex(value.intContainsAny(asList(100, 200, 300, 400, 500)));
        ...
        ulElementsPage.webList()
                .extractAll(componentDisplayedMark(blockMock(CountryNameBlock.class).fullName(), "SNG"))
                .shouldHaveSize(value.intEquals(193));
        ...
        ulElementsPage.webList()
                .extractAll(textValue(blockMock(CountryNameBlock.class).shortName()))
        ...
        ulElementsPage.webList()
                .filter(with(containsText(blockMock(CountryNameBlock.class).shortName(), value.stringContains("gia"))))
        ...
        ulElementsPage.webList()
                .filter(with(containsText(blockMock(CountryNameBlock.class).number(), value.intLessThanOrEqual(77))))
        ...
        WebListFilter elementPropertyValueFilter = with(containsProperty(blockMock(CountryNameBlock.class).shortName(), "Wiki link",
        ...
        MultipleResult<CountryNameBlock> filteredByElementComponentDisplayedBlocks = ulElementsPage.webList()
                .filter(with(componentDisplayed(blockMock(CountryNameBlock.class).fullName(), "SNG")))
                .extractAll(block(CountryNameBlock.class));

        // Case: Получить все экземпляр блоков, в которых номер страны больше 77 и короткое имя страны НЕ содержит 'na'
        WebListFilter multipleFilter = with(containsText(blockMock(CountryNameBlock.class).number(), value.intGreaterThan(77)))
                .subtract(containsText(blockMock(CountryNameBlock.class).shortName(), value.stringContains("na")));
