...
//                .shouldHaveIndex(value.intContainsAny(asList(100, 200, 300, 400, 500)));
        ...
        tablePage.table()
                .extractAllRows(WebExtractors.element(TablePage.CHECKBOX, blockMock(CheckboxWebMappedBlock.class).checkbox()))
                .shouldHaveSize(value.intEquals(193));

        // Case: Получить полные названия стран и проверить из количество
        tablePage.table()
                .extractAllRows(WebExtractors.element(TablePage.FULL_NAME, blockMock(FullNameWebMappedBlock.class).fullName()))
                .shouldHaveSize(value.intEquals(193));

        // Case: Получить номера стран в списке и проверить их количество
        tablePage.table()
                .extractAllRows(WebExtractors.textValue(TablePage.NUMBER, blockMock(SimpleStringWebMappedBlock.class).text()))
                .shouldHaveSize(value.intEquals(193));

        // Case: Получить признак отображения элементов с полным именем страны и проверить их количество
        tablePage.table()
                .extractAllRows(WebExtractors.displayedMark(TablePage.FULL_NAME, blockMock(FullNameWebMappedBlock.class).fullName()))
                .shouldHaveSize(value.intEquals(193));

        // Case: Получить признак доступности чекбокса для выделения и проверить их количество
        tablePage.table()
                .extractAllRows(WebExtractors.enabledMark(TablePage.CHECKBOX, blockMock(CheckboxWebMappedBlock.class).checkbox()))
                .shouldHaveSize(value.intEquals(193));

        // Case: Получить признак выделения чекбокса и проверить их количество
        tablePage.table()
                .extractAllRows(WebExtractors.selectedMark(TablePage.CHECKBOX, blockMock(CheckboxWebMappedBlock.class).checkbox()))
                .shouldHaveSize(value.intEquals(193));

        // Case: Получить значения свойства 'Wiki link' для короткого имени страны и проверить их количество
        tablePage.table()
                .extractAllRows(WebExtractors.propertyValue(TablePage.SHORT_NAME, blockMock(LinkWebMappedBlock.class).shortName(), "Wiki link"))
                .shouldHaveSize(value.intEquals(193));

        // Case: Получить признак состояния 'SNG' элемента 'English name' и проверить их количество
        tablePage.table()
                .extractAllRows(WebExtractors.componentDisplayedMark(TablePage.FULL_NAME, blockMock(FullNameWebMappedBlock.class).fullName(), "SNG"))
                .shouldHaveSize(value.intEquals(193));
        ...
        tablePage.table()
                .extractAllRows(WebExtractors.textValue(TablePage.SHORT_NAME, blockMock(LinkWebMappedBlock.class).shortName()))
        ...
        MultipleResult<FullNameWebMappedBlock> filteredByElementDisplayedBlocks = tablePage.table()
                .filter(with(WebConditions.displayed(TablePage.FULL_NAME, blockMock(FullNameWebMappedBlock.class).fullName())))
                .shouldHaveSize(value.intEquals(193))
        ...
        tablePage.table()
                .filter(with(WebConditions.containsText(TablePage.SHORT_NAME, blockMock(LinkWebMappedBlock.class).shortName(), value.stringContains("gia"))))
        ...
        tablePage.table()
                .filter(with(WebConditions.containsText(TablePage.NUMBER, blockMock(SimpleStringWebMappedBlock.class).text(), value.intLessThanOrEqual(77))))
        ...
        MultipleResult<WebCheckbox> filteredByElementEnabledCheckboxes = tablePage.table()
                .filter(with(WebConditions.enabled(TablePage.CHECKBOX, blockMock(CheckboxWebMappedBlock.class).checkbox())))
                .extractAllRows(WebExtractors.element(TablePage.CHECKBOX, blockMock(CheckboxWebMappedBlock.class).checkbox()));

        // Case: Проверить количество выделенных чекбоксов
        tablePage.table()
                .filter(with(WebConditions.selected(TablePage.CHECKBOX, blockMock(CheckboxWebMappedBlock.class).checkbox())))
        ...
        WebTableFilter elementPropertyValueFilter = with(WebConditions.containsProperty(TablePage.SHORT_NAME, blockMock(LinkWebMappedBlock.class).shortName(),
        ...
        MultipleResult<FullNameWebMappedBlock> filteredByElementComponentDisplayedBlocks = tablePage.table()
                .filter(with(WebConditions.componentDisplayed(TablePage.FULL_NAME, blockMock(FullNameWebMappedBlock.class).fullName(), "SNG")))
                .extractAllRows(cell(TablePage.FULL_NAME, FullNameWebMappedBlock.class));

        // Case: Получить все экземпляр ячеек, в которых номер страны больше 77 и короткое имя страны НЕ содержит 'na'
        WebTableFilter multipleFilter = with(WebConditions.containsText(TablePage.NUMBER, blockMock(SimpleStringWebMappedBlock.class).text(), value.intGreaterThan(77)))
                .subtract(WebConditions.containsText(TablePage.SHORT_NAME, blockMock(LinkWebMappedBlock.class).shortName(), value.stringContains("na")));
