...
public class StringContainsAnyChecker extends AbstractStringChecker {
    ...
    public boolean check() {
        return expectedValues.stream()
                .map(this::processExpectedValue)
                .map(this::applyTransformersToExpected)
                .anyMatch(getProcessedActual()::contains);
    }

    @Override
    public @NotNull String getShortComparisonDescription() {
        return String.format("contains any %s", getProcessedExpected());
    }
    ...
    public @NotNull String getComparisonDescription() {
        return String.format("Compare parameters:\n"
                        ...
                        + "    processedExpected: '%s'\n"
                        + "      processedActual: '%s'",
                getExpected(), getActual(), this.getClass().getCanonicalName(), "contains any", getProcessedExpected(), getProcessedActual());
    }
