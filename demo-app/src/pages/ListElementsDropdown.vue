<template>
    <div id="dropdown-list-elements-page">
        <b-container>
            <b-row>
            ...
            <b-row class="mb-3 text-center" align-v="center">
                <b-col cols="12">
                    <b-dropdown class="dropdown-popup" id="drop-down-countries-list" variant="outline-primary" block :text="dropDownSelectedValue ? dropDownSelectedValue : 'Select country'">
                        <b-dropdown-item itemid="countries-list-item" v-for="(country_ddl, i) in countriesData" :key="'ddl' + i" v-on:click="dropDownSelectedValue=country_ddl.name">
                            <b-row>
                                <b-col cols="1">
                                    <b-checkbox itemid="checkbox" :id="'ddl' + country_ddl.name" :checked="country_ddl.selected" :disabled="!country_ddl.enabled"></b-checkbox>
                                </b-col>
                                <b-col cols="1">
                                    <span itemid="number">{{ country_ddl.number }}</span>
                                </b-col>
                                <b-col cols="3">
                                    <b-link itemid="name" class="mr-2" v-b-tooltip.hover :title="country_ddl.name" :href="country_ddl.wikiLink">{{
                                            formatName(country_ddl.name, 22) }}
                                    </b-link>
                                </b-col>
                                <b-col cols="5">
<!--                                    <span itemid="full-name" class="mr-2" v-b-tooltip.hover :title="country_ddl.fullName">{{ formatName(country_ddl.fullName, 38) }}-->
                                    <span itemid="full-name" class="mr-2" :title="country_ddl.fullName">{{ formatName(country_ddl.fullName, 38) }}
                                        <!-- Not present -->
                                        <b-badge class="ml-2" itemid="sng" v-if="country_ddl.sng">СНГ</b-badge>
                                    </span>
                                </b-col>
                                <b-col cols="2">
                                    <!-- Not present -->
                                    <span itemid="population-number" v-if="country_ddl.population">{{ formatPopulation(country_ddl.population) }}</span>
                                    <!-- Not displayed -->
                                    <span itemid="population-unit" v-show="country_ddl.population">&nbsp;тыс</span>
                                </b-col>
                            </b-row>
                        </b-dropdown-item>
                    </b-dropdown>
