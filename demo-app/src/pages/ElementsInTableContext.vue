<template>
    <div id="elements-cell-context-page">
        <b-container>
            <b-row>
            ...
            <b-row class="mb-3 text-center" align-v="center">
                <b-col cols="12">
                    <b-table id="continents-table" striped hover outlined :items="continentsData" :fields="continentsFields">
                        <!-- Selected checkbox -->
                        <template v-slot:cell(name)="data">
                            <span itemid="continent-name" v-if="data.item.name!==null" >{{ data.item.name }}</span>
                        </template>
                        <template v-slot:cell(total)="data">
                            <b-badge itemid="continent-total" variant="primary" v-b-tooltip.hover :title="data.item.total" pill>{{ data.item.total }}</b-badge>
                        </template>
                        <template v-slot:cell(countries)="data">
                            <!-- Inner table -->
                            <b-table itemid="countries-table" striped hover outlined :items="data.item.countries" :fields="countriesFields">
                                <!-- Selected checkbox -->
                                ...
                                <template v-slot:cell(fullName)="data">
                                    <span itemid="country-full-name" v-b-tooltip.hover :title="data.item.fullName">{{ data.item.fullName }}
                                        <!-- Not present -->
                                        <b-badge class="ml-2" itemid="sng" v-if="data.item.sng">СНГ</b-badge>
                                    </span>
                                </template>
                                <template v-slot:cell(population)="data">
                                    <!-- Not present -->
                                    <span itemid="population-number" v-if="data.item.population">{{ formatPopulation(data.item.population) }}</span>
                                    <!-- Not displayed -->
                                    <span itemid="population-unit" v-show="data.item.population">&nbsp;тыс</span>
                                </template>
                            </b-table>
                        </template>
                    </b-table>
...
<script>
    export default {
        data: function() {
            return {
                pageTitle: 'Таблица в таблице',
                toastCount: 195,
                continentsFields: [
                    {key: 'name', sortable: true},
                    {key: 'total', sortable: true},
                    {key: 'countries', sortable: false},
                ],
                countriesFields: [
                ...
                continentsData: [
                    {
                        ...
                        countries: [
                            ...
                            {number: 4, name: '', fullName: '', sng: false, population: null, wikiLink: null, selected: false, enabled: true},
                            {
                                number: 5,
                    ...
                    },
                    {
                        ...
                        countries: [
                            ...
                            {number: 39, name: '', fullName: '', sng: false, population: null, wikiLink: null, selected: false, enabled: true},
                            {
                                number: 40,
                                name: 'Мадагаскар',
                    ...
                    },
                    {
                        ...
                        countries: [
                            ...
                            {number: 12, name: '', fullName: '', sng: false, population: null, wikiLink: null, selected: false, enabled: true},
                            {
                                number: 13,
                                name: 'Самоа',
