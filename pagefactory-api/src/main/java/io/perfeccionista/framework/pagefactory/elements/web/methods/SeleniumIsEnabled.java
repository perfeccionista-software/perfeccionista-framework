...
public class SeleniumIsEnabled implements ElementMethodImplementation<WebChildElement, Boolean> {

    public OperationResult<Boolean> execute(WebChildElement element, Object... args) {
        return OperationResult.execute(() -> {
            JsOperation<SingleResult<WebElement>> operation = JsOperation.single(element.getLocatorChainTo(ENABLED), GetWebElement.class);
