...
@CucumberResolverExpression("case-insensitive substring")
@CucumberResolverExpression({"подстрока без учета регистра", "подстроку без учета регистра"})
public class StringContainsIgnoreCaseValueCheckerCucumberResolver extends AbstractStringCheckerCucumberResolver {
        ...
        for (String pattern : patterns) {
            if (stringValueCondition.contains(pattern)) {
                return Optional.of(environment.getService(ValueService.class)
                        .stringContainsIgnoreCase(extractFirstNotNullStringArgument(args)));
            }
        }
